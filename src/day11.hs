...
runPainter :: Board -> (Int, Int) -> Orientation -> [Int] -> [(Int, (Int, Int))]
runPainter _ _ _ [] = []
runPainter state pos ort (color:turn:rest) =
  ...
  in
    (nextColor, nextPos) : runPainter nextState nextPos nextOrt rest

runPainter _ _ _ _ = error "unexpected end of input"

process :: [Int] -> Int
process program =
  let robotInput = map fst painterOutput
      robotOutput = run program robotInput
      painterOutput = (0, (0, 0)) : (runPainter M.empty (0, 0) Up robotOutput)
  in
    length $ nubOrd $ map snd $ init painterOutput
